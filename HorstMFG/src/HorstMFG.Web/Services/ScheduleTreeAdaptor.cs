...
public class ScheduleTreeAdaptor : DataAdaptor
{
    ...
    public override async Task<object> ReadAsync(DataManagerRequest dm, string key = null!)
    {
        ...
        List<ExportTreeItem> items = parentTreeId.HasValue
            ? await _bomService.GetScheduleChildrenByParentTreeIdAsync(parentTreeId.Value)
            : await _bomService.GetScheduleTreeItemsAsync(_state.PlantId, _state.FromDate, _state.ToDate, _state.SearchTerm);

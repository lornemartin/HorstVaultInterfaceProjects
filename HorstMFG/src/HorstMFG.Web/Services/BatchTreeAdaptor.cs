...
public class BatchTreeAdaptor : DataAdaptor
{
    ...
    public override async Task<object> ReadAsync(DataManagerRequest dm, string key = null!)
    {
        ...
        List<ExportTreeItem> items = parentTreeId.HasValue
            ? await _bomService.GetBatchChildrenByParentTreeIdAsync(parentTreeId.Value)
            : await _bomService.GetBatchTreeItemsAsync(_state.PlantId, _state.FromDate, _state.ToDate, _state.SearchTerm);
